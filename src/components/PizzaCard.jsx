...
function PizzaCard({
  ...
  const priceCents = Number.isFinite(Number(pizza.priceCents))
    ? Number(pizza.priceCents)
    : Math.round(
        (parseFloat(String(pizza.price || '0').replace(',', '.')) || 0) * 100
      );
  ...
  const displayName =
    language === 'en' && pizza.nameEn ? pizza.nameEn : pizza.name;

  const displayDescription =
    language === 'en' && pizza.descriptionEn
      ? pizza.descriptionEn
      : pizza.description;
  ...
  const addToCartLabel =
    language === 'en'
      ...
      : `Lisää ${displayName} koriin`;

  const decreaseLabel =
    language === 'en'
      ? `Decrease ${displayName}`
      : `Vähennä tuotetta ${displayName}`;
  ...
  const tagLabel =
    Array.isArray(pizza.diet) && pizza.diet.includes('VEG')
      ? language === 'en'
        ? 'Veg'
        : 'Vege'
      : language === 'en'
  ...
  function spawnStars() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const rot = Math.round(
        Math.random() * 540 * (Math.random() < 0.5 ? -1 : 1)
      );
      ...
      star.style.boxShadow = `0 0 ${Math.max(
        6,
        Math.round(size * 0.8)
      )}px ${color}`;
  ...
  return (
    <article
      ref={cardRef}
      className={`pizza-card${highlighted ? ' pizza-card--highlighted' : ''}`}
      id={anchorId}
    >
      <button
        type="button"
      ...
      >
        ...
        <img
          src={resolveImageUrl(pizza.image)}
          alt={`${displayName} pizza`}
          loading="lazy"
        />
      </button>
      ...
      <div className="pizza-card__content">
        {cartLimitReached ? (
          <div className="pizza-card__limit" aria-hidden="true">
            {cartLimitMessage}
          </div>
        ) : null}

        <div className="pizza-card__topline">
          <span className="pizza-card__tag">{tagLabel}</span>
        </div>
        ...
        <div className="pizza-card__meta">
          <div className="pizza-card__price-wrap">
            {offerActive ? (
              <>
                <span className="pizza-card__price-old">
                  {formatEuro(priceCents)}
                </span>
                <span className="pizza-card__price-discount">
                  {formatEuro(discountedCents)}
                </span>
              </>
            ) : (
              <span className="pizza-card__price-current">
                {formatEuro(priceCents)}
              </span>
            )}
          </div>
          ...
          <div className="pizza-card__actions">
            ...
            {isInCart ? (
              <div
                ...
                aria-label={
                  language === 'en'
                    ? `${displayName} quantity controls`
                    : `${displayName} määrän säätö`
                }
              >
                  ...
                  {cartQuantity >= 2 ? (
                    <>
                      {offerActive && priceCents > discountedCents ? (
                        <div className="pizza-card__save">
                          {language === 'en'
                            ? `You save ${formatEuro(
                                (priceCents - discountedCents) * cartQuantity
                              )}`
                            : `Säästät ${formatEuro(
                                (priceCents - discountedCents) * cartQuantity
                              )}`}
                      ...
                      <div className="pizza-card__total">
                        {language === 'en' ? 'Total' : 'Yhteensä'}{' '}
                        {formatEuro(
                          (offerActive ? discountedCents : priceCents) *
                            cartQuantity
                        )}
                      </div>
                    </>
              ...
              <button
                ...
                onClick={handleAdd}
              >
                {language === 'en' ? 'Add to cart' : 'Lisää koriin'}
              </button>
            )}
          </div>
        </div>
